...
public class PortalHooks : ModSystem
{
    /// <summary> Identify the target item. </summary>
    internal static bool IsPortalCreatorItem(Item item) => !item.IsAir && item.type == ModContent.ItemType<PortalCreatorItem>();
    ...
    private bool OverrideIgnoreMouseInterface(Func<bool> orig)
    {
        ...
        if (p != null &&
            ...
            p.itemAnimation > 0 &&
            IsPortalCreatorItem(p.HeldItem))
        {
    ...
    private static void Modify_RightClick(On_ItemSlot.orig_RightClick_ItemArray_int_int orig, Item[] inv, int context, int slot)
    {
        ...
        if (player.itemAnimation > 0 && !IsPortalCreatorItem(player.HeldItem))
        {
            return;
        }
        if (context == 15)
    ...
    private void Modify_DropSelectedItem(On_Player.orig_DropSelectedItem orig, Player self)
    {
        ...
        if (IsPortalCreatorItem(selectedItem))
        {
            return;
        }
        orig(self);
    ...
    private static bool Modify_LeftClick_SellOrTrash(On_ItemSlot.orig_LeftClick_SellOrTrash orig, Item[] inv, int context, int slot)
    {
        ...
        if (!item.IsAir && item.type == ModContent.ItemType<PortalCreatorItem>())
        {
            //if (Main.npcShop > 0)
    ...
    private static void Modify_LeftClick_ItemArray(On_ItemSlot.orig_LeftClick_ItemArray_int_int orig, Item[] inv, int context, int slot)
    {
        ...
        if (slot >= 0 && slot < inv.Length && IsPortalCreatorItem(inv[slot]))
        {
        ...
        if (context == ItemSlot.Context.ChestItem && IsPortalCreatorItem(Main.mouseItem)
        ...
        bool bypassAnimCheck = IsPortalCreatorItem(player.HeldItem);
...
internal sealed class DisablePickupWhileHoldingMirror : GlobalItem
{
    public override bool CanPickup(Item item, Player player)
    {
        ...
        if (player.HeldItem.type == ModContent.ItemType<PortalCreatorItem>() && freeSlots <= 1)
        {
            return false;
        }
